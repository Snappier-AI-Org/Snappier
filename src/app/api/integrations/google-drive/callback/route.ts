  ...
  try {
    ...
    const existingCredential = await prisma.credential.findFirst({
      where: {
        ...
        // We can't easily search inside encrypted value, so we'll create a new one
        // or update based on name pattern
      },
    });
    ...
    return NextResponse.redirect(
      new URL("/credentials?success=google_drive_connected", req.url)
    );
  } catch (error) {
