...
export const GoogleSheetsNode = memo((props: NodeProps<GoogleSheetsNodeType>) => {
    ...
    // Log status changes
    if (nodeStatus === "error") {
      console.log(`[Google Sheets Node ${props.id}] Node status is error`, {
        nodeId: props.id,
    ...
    if (errorMessage) {
      console.log(`[Google Sheets Node ${props.id}] Error message extracted:`, {
        errorMessage,
        errorGuidance,
        errorFixSteps,
    ...
    const description = nodeData?.spreadsheetId
        ? `${operation.charAt(0).toUpperCase() + operation.slice(1)}: ${nodeData.spreadsheetId.slice(0, 20)}...`
        : "Not configured";
