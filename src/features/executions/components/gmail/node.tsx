...
type GmailNodeData = {
  credentialId?: string;
  operation?: "list" | "read" | "send" | "reply" | "draft" | "search" | "delete" | "label";
  to?: string;
  cc?: string;
  bcc?: string;
  subject?: string;
...
export const GmailNode = memo((props: NodeProps<GmailNodeType>) => {
  ...
  // Log status changes
  if (nodeStatus === "error") {
    console.log(`[Gmail Node ${props.id}] Node status is error`, {
      nodeId: props.id,
  ...
  if (errorMessage) {
    console.log(`[Gmail Node ${props.id}] Error message extracted:`, {
      errorMessage,
      errorGuidance,
      errorFixSteps,
  ...
  const getOperationLabel = (op: string) => {
    switch (op) {
      case "list": return "List Messages";
