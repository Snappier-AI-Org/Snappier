...
const parseOpenRouterError = (error: unknown) =>
  parseError(error, { provider: "openrouter" });
...
export const openrouterExecutor: NodeExecutor<OpenRouterData> = async ({
  ...
  if (!data.variableName) {
    const parsedError = parseOpenRouterError(new Error("Variable name is missing"));
    await publish(
      openrouterChannel().status({
        nodeId,
        status: "error",
  ...
  if (!data.credentialId) {
    const parsedError = parseOpenRouterError(new Error("Credential is required"));
    await publish(
      openrouterChannel().status({
        nodeId,
        status: "error",
  ...
  if (!data.userPrompt) {
    const parsedError = parseOpenRouterError(new Error("User prompt is missing"));
    await publish(
      openrouterChannel().status({
        nodeId,
        status: "error",
  ...
  if (data.userPrompt) {
    debugTemplateContext("OpenRouter Node", data.userPrompt, context);
  }
  ...
  if (!credential) {
    const parsedError = parseOpenRouterError(new Error("Credential not found"));
    await publish(
      openrouterChannel().status({
        nodeId,
        status: "error",
  ...
  try {
    ...
    const parsedError = parseOpenRouterError(decryptError);
    await publish(
      openrouterChannel().status({
        nodeId,
        status: "error",
  ...
  try {
    ...
    const result = await step.ai.wrap("openrouter-generate-text", generateText, {
    ...
    if (
      ...
      console.error(`[OpenRouter Executor] Model identifier error for model "${selectedModel}":`, errorMessage);
    }
