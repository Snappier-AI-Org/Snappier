...
export const WhatsAppNode = memo((props: NodeProps<WhatsAppNodeType>) => {
  ...
  useEffect(() => {
    ...
    if (previousStatusRef.current !== executionStatus) {
      console.log(`[WhatsApp Node ${props.id}] Execution status changed:`, {
        nodeId: props.id,
        executionId,
    ...
    if (executionId !== previousExecutionIdRef.current && executionStatus === "SUCCESS") {
      console.log(`[WhatsApp Node ${props.id}] ✅ Execution completed successfully!`, {
        nodeId: props.id,
        executionId,
        nodeData: props.data,
      ...
      // If output contains our variable, log the WhatsApp response details
      if (latestExecution.output && props.data?.variableName && typeof latestExecution.output === 'object' && latestExecution.output !== null) {
        ...
        const variableData = output[props.data.variableName] as { messageId?: string; messageContent?: string } | undefined;
        if (variableData) {
          console.log(`[WhatsApp Node ${props.id}] 📨 WhatsApp message details:`, {
    ...
    if (executionStatus === "FAILED" && latestExecution.error) {
      console.error(`[WhatsApp Node ${props.id}] ❌ Execution failed:`, {
        nodeId: props.id,
        executionId,
        error: latestExecution.error,
  ...
  // Log status changes
  if (nodeStatus === "error") {
    console.log(`[WhatsApp Node ${props.id}] Node status is error`, {
      nodeId: props.id,
  ...
  useEffect(() => {
    if (nodeStatus === "success") {
      console.log(`[WhatsApp Node ${props.id}] ✅ Node status: success`, {
        nodeId: props.id,
  ...
  if (errorMessage) {
    console.log(`[WhatsApp Node ${props.id}] Error message extracted:`, {
      errorMessage,
      errorGuidance,
      errorFixSteps,
