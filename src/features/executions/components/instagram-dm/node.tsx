...
import { fetchInstagramDmRealtimeToken } from "./actions";
import { INSTAGRAM_DM_CHANNEL_NAME } from "@/inngest/channels/instagram-dm";
import { InstagramDmDialog, InstagramDmFormValues } from "./dialog";
...
type InstagramDmNodeType = Node<InstagramDmNodeData>;

export const InstagramDmNode = memo((props: NodeProps<InstagramDmNodeType>) => {
  ...
  const nodeStatus = useNodeStatus({
    nodeId: props.id,
    channel: INSTAGRAM_DM_CHANNEL_NAME,
    topic: "status",
    refreshToken: fetchInstagramDmRealtimeToken,
  });
  ...
  const handleSubmit = (values: InstagramDmFormValues) => {
    setNodes((nodes) =>
      nodes.map((node) => {
        if (node.id === props.id) {
          return {
  ...
  const description = nodeData?.messageContent
    ? `"Send": ${nodeData.messageContent.slice(0, 50)}...`
    : "Not configured";
