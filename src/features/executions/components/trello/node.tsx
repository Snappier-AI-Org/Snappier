...
import { fetchTrelloRealtimeToken } from "./actions";
import { TRELLO_CHANNEL_NAME } from "@/inngest/channels/trello";
import { TrelloDialog, TrelloFormValues } from "./dialog";
...
type TrelloNodeData = {
  credentialId?: string;
  operation?: "createCard" | "getCard" | "updateCard" | "moveCard" | "deleteCard" | "addComment" | "searchCards";
  boardId?: string;
  listId?: string;
  cardId?: string;
  cardName?: string;
...
export const TrelloNode = memo((props: NodeProps<TrelloNodeType>) => {
  ...
  const nodeStatus = useNodeStatus({
    nodeId: props.id,
    channel: TRELLO_CHANNEL_NAME,
    topic: "status",
    refreshToken: fetchTrelloRealtimeToken,
  });
  ...
  const operation = nodeData?.operation || "createCard";
