...
import { fetchInstagramCommentReplyRealtimeToken } from "./actions";
import { INSTAGRAM_COMMENT_REPLY_CHANNEL_NAME } from "@/inngest/channels/instagram-comment-reply";
import {
  InstagramCommentReplyDialog,
  InstagramCommentReplyFormValues,
} from "./dialog";
...
type InstagramCommentReplyNodeData = {
  credentialId?: string;
  commentId?: string;
  replyText?: string;
  variableName?: string;
...
type InstagramCommentReplyNodeType = Node<InstagramCommentReplyNodeData>;

export const InstagramCommentReplyNode = memo(
  (props: NodeProps<InstagramCommentReplyNodeType>) => {
    ...
    const nodeStatus = useNodeStatus({
      nodeId: props.id,
      channel: INSTAGRAM_COMMENT_REPLY_CHANNEL_NAME,
      topic: "status",
      refreshToken: fetchInstagramCommentReplyRealtimeToken,
    });
    ...
    const handleSubmit = (values: InstagramCommentReplyFormValues) => {
      setNodes((nodes) =>
        nodes.map((node) => {
          if (node.id === props.id) {
            return {
    ...
    const description = nodeData?.replyText
      ? `"Reply": ${nodeData.replyText.slice(0, 50)}...`
      : "Not configured";
...
InstagramCommentReplyNode.displayName = "InstagramCommentReplyNode";
